...
export default function CampaignOwenrList() {
  ...
  const handleUpdateProcess = useCallback(async (id) => {
    try {
      const response = await axios.put(
        `http://ithcm2024-001-site1.gtempurl.com/index.html/api/v1/campaigns/update-process`,
        {
          headers: {
  ...
  const handleUpdateRelust = useCallback(async (id) => {
    try {
      const response = await axios.put(
        `http://ithcm2024-001-site1.gtempurl.com/index.html/api/v1/campaigns/publish-result/6097a517-11ad-4105-b26a-0e93bea2cb43`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
